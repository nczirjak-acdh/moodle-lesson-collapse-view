...
$mode    = optional_param('mode',get_user_preferences('lesson_view', 'full'), PARAM_ALPHA);
...
if ($backtocourse) {
    redirect(new moodle_url('/course/view.php', array('id'=>$course->id, 'mode' => 'full')));
}
...
$PAGE->set_url($url,array('id'=>$cm->id,'mode'=>$mode));
...
if (empty($pageid)) {
    ...
    if ($branchtables = $DB->get_records('lesson_branch', array("lessonid" => $lesson->id, "userid" => $USER->id, "retry" => $retries), 'timeseen ASC')) {
    ...
    //31.08.2016 - The lesson continue feature disabled - By Norbert Czirjak
...
*/
    ...
    if (is_array($page->answers) && count($page->answers)>0) {
        ...
        /* 18.10.2016 Norbert Czirjak - expanded view to the lessons */
        $lessoncontent = $lessonoutput->display_page_full($lesson, $pageid, 0, false, $page, $attempt);
        /* 18.10.2016 Norbert Czirjak - the old default lessons view */        
